...
const SKILL_DEFS = [
  { skill: "Programming Logic", max: 20 },
  { skill: "Circuit Assembly", max: 20 },
  { skill: "Soldering Quality", max: 20 },
  { skill: "3D Design Skills", max: 20 },
  { skill: "Team Collaboration", max: 20 },
];

const initialGrades = (): StudentGrade[] => [
  { id: "STU-801", name: "Amine Bouaziz", skills: SKILL_DEFS.map(s => ({ ...s, score: 0 })) },
  { id: "STU-802", name: "Yasmine Haddad", skills: SKILL_DEFS.map(s => ({ ...s, score: 0 })) },
  { id: "STU-803", name: "Karim Benamar", skills: SKILL_DEFS.map(s => ({ ...s, score: 0 })) },
  { id: "STU-804", name: "Sara Merabet", skills: SKILL_DEFS.map(s => ({ ...s, score: 0 })) },
];
...
export default function GradesEntry() {
  ...
  const getGrade = (pct: number) => {
    if (pct >= 90) return { label: "Excellent", color: "text-emerald-glow" };
    if (pct >= 75) return { label: "Very Good", color: "text-cyber-cyan" };
    if (pct >= 60) return { label: "Good", color: "text-neon-purple" };
    if (pct >= 50) return { label: "Average", color: "text-laser-amber" };
    return { label: "Needs Work", color: "text-neon-red" };
  };
    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-obsidian-800 pb-6">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">Log student performance scores per skill domain. Max total: {getMaxTotal()} points.</p>
        </div>
        <button
          onClick={() => setSaved(true)}
          className="flex items-center gap-2 px-5 py-2.5 text-xs font-bold rounded-xl border border-emerald-glow/40 bg-emerald-glow/5 text-emerald-glow hover:bg-emerald-glow/10 transition-all cursor-pointer"
        >
      ...
      {saved && (
        <div className="p-4 rounded-2xl bg-emerald-glow/5 border border-emerald-glow/20 flex items-center gap-3 text-xs">
          ...
          <span className="text-emerald-glow font-bold">Grade registry saved and committed to student profiles.</span>
        </div>
      )}
            ...
            <GlassCard key={student.id} hoverable={false} className="p-6 space-y-5">
              <div className="flex justify-between items-center">
                ...
                <div className="text-right">
                  <p className="text-2xl font-extrabold font-mono text-white">{total}<span className="text-gray-500 text-sm">/{maxTotal}</span></p>
                  <p className={cn("text-xs font-bold", grade.color)}>{grade.label} • {pct}%</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 text-xs">
                {student.skills.map(skill => (
                  <div key={skill.skill} className="space-y-1.5">
                    <label className="block text-gray-400 font-semibold text-[10px] leading-tight">{skill.skill}</label>
